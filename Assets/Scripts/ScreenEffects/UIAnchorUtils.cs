...
public class UIAnchorUtils
{
    ...
    public static void uGUIAnchorAroundObjectFinal(GameObject go = null)
    {
        ...
        if (gameObjectToUpdateAnchor != null &&
        gameObjectToUpdateAnchor.GetComponent<RectTransform>() != null)
        {
            Undo.RecordObject(gameObjectToUpdateAnchor, "Reasigned Anchor");
            var r = gameObjectToUpdateAnchor.GetComponent<RectTransform>();
            var p =
            gameObjectToUpdateAnchor.transform.parent.GetComponent<RectTransform>();
            ...
            var anchorMin = new Vector2(_anchorMin.x + (offsetMin.x /
            parent_width),
            _anchorMin.y + (offsetMin.y /
            parent_height));
            var anchorMax = new Vector2(_anchorMax.x + (offsetMax.x /
            parent_width),
            _anchorMax.y + (offsetMax.y /
            parent_height));
    ...
    static void uGUIAnchorAroundObjectAll()
    {
        ...
        foreach (GameObject go in Selection.objects)
        {
            if (!gameObjectsToUpdate.Contains(go.transform))
            {
                ...
                Transform[] childrensGameObjects =
                ...
                foreach (Transform childrenGo in childrensGameObjects)
                {
                    ...
                    {
                        if (childrenGo.parent != null && !
                        childrenGo.parent.GetComponent<TMPro.TextMeshProUGUI>())
